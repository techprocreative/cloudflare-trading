...
const customTools = [
  {
    ...
    function: {
      ...
      description: 'Gets REAL market data and a trading signal for a given currency pair, stock, or cryptocurrency.',
      parameters: {
        type: 'object',
        properties: {
          pair: {
            type: 'string',
            description: 'The currency pair or stock symbol to analyze. Examples: "USD/IDR", "BBCA", "BTC/USD", "EUR/IDR".',
          },
        },
        required: ['pair'],
      },
...
*Indonesian market symbols only*`
      ...
      case 'execute_trade_signal': {
        ...
        try {
          ...
          return {
            success,
            message: success ? `Trade executed successfully at ${marketData.price.toFixed(4)}.` : `Trade failed due to simulated market volatility.`,
            accountId,
            pair,
            action,
            amount,
