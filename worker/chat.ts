...
export class ChatHandler {
  ...
  async processMessage(
    ...
    try {
      ...
      if (error instanceof OpenAI.APIError && error.status === 400) {
        console.warn("AI Gateway not configured. Using mock response.");
        return { content: MOCK_MODE_MESSAGE };
      }
  ...
  private async generateToolResponse(
    ...
    try {
      ...
      if (error instanceof OpenAI.APIError && error.status === 400) {
        console.warn("AI Gateway not configured for follow-up. Using mock response.");
        return MOCK_MODE_MESSAGE;
      }
