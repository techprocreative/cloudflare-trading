...
export function useSubscription() {
  ...
  return {
    getSubscriptionInfo: () => user?.user?.id ? subscriptionManager.getSubscriptionInfo(user.user.id) : null,
    canMakeAIRequest: () => user?.user?.id ? subscriptionManager.canMakeAIRequest(user.user.id) : { allowed: false },
    canGenerateSignal: () => user?.user?.id ? subscriptionManager.canGenerateSignal(user.user.id) : { allowed: false },
    hasFeature: (feature: string) => user?.user?.id ? subscriptionManager.hasFeature(user.user.id, feature) : false,
    getDataDelay: () => user?.user?.id ? subscriptionManager.getDataDelay(user.user.id) : '15 minutes',
    getMaxSymbols: () => user?.user?.id ? subscriptionManager.getMaxSymbols(user.user.id) : 5,
    recordAIRequest: () => user?.user?.id && subscriptionManager.recordAIRequest(user.user.id),
    recordSignal: () => user?.user?.id && subscriptionManager.recordSignal(user.user.id)
  };
}
